...
function errorMessage(error) {
  ...
  if (name === 'NotAllowedError') return '未允許使用咪高峰，請到 iPhone 設定 → Safari → 咪高峰，選擇允許。';
  if (name === 'NotSupportedError') return '這部裝置暫時不支援網頁錄音，請用最新版 Safari。';
  if (code.includes('storage/quota-exceeded') || message.includes('402')) return '影片上載需要先啟用 Firebase Storage（Blaze 方案）。圖片及短錄音仍可用免費模式傳送。';
  if (code.includes('storage/bucket-not-found') || code.includes('storage/object-not-found')) return '找不到 Firebase Storage 儲存空間，請先在 Firebase Console 建立 Storage bucket。';
  if (code.includes('storage/unauthorized')) return 'Storage 權限未設定完成，請在 Firebase Storage 發布最新 Rules。';
  if (code.includes('storage/retry-limit-exceeded')) return '上載逾時，請檢查網絡後再試。';
  if (code.includes('resource-exhausted') || message.includes('maximum size')) return '媒體檔案太大，請選擇較小圖片或重新錄製。';
  if (code.includes('permission-denied')) return '資料庫權限未設定完成，請發布最新 Firestore Rules。';
...
async function playTone() {
  ...
  try {
    ...
    oscillator.start();
    oscillator.stop(S.audioContext.currentTime + 0.23);
  } catch (error) {
...
function updateCoupleHeader() {
  const connected = S.members.length === 2;
  const roomNumber = CS.rooms.currentRoomNumber?.();
  $('space-title').textContent = roomNumber
    ? `二人空間 ${roomNumber} 💞`
    : (connected ? '我哋嘅二人空間 💞' : '等待另一半加入 💗');
  $('connection-status').textContent = connected ? '● 另一半已連線' : '○ 尚未完成配對';
  const inviteCode = CS.rooms.currentInviteCode?.() || S.profile.inviteCode || '';
  $('invite-card').classList.toggle('hidden', connected || !inviteCode);
