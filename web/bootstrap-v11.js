...
function bindUi() {
  ...
  $('join-button').addEventListener('click', () => CS.chat.joinCouple('join-code'));
  ...
  $('record-button').addEventListener('click', () => {
    CS.media.startRecording().catch((error) => showToast(errorMessage(error), 6000));
  });
  ...
  $('copy-code').addEventListener('click', async () => {
    await navigator.clipboard.writeText(CS.rooms.currentInviteCode() || '');
    showToast('配對碼已複製');
  });
...
async function bootstrap() {
  ...
  const bucketName = String(CS.firebaseConfig.storageBucket || '').replace(/^gs:\/\//, '');
  S.storage = bucketName ? getStorage(app, `gs://${bucketName}`) : getStorage(app);
